...
public class RenderController implements IDisposable {
	...
	/**
	 * update function for final project
	 * @param r
	 * @param camUpdater
	 */
	public void update(Renderer r, CameraUpdater camUpdater) {
		ArrayList<SceneEvent> le = new ArrayList<>();
		queue.getEvents(le);
		if(le.size() == 0) return;
		...
		if(isTreeModified) {
			...
			if(camUpdater.camera != null) {
				SceneObject so = camUpdater.camera.sceneObject;
				for(RenderCamera cam : env.cameras ) {
					if(cam.sceneObject == so) {
				...
				camUpdater.camera = env.cameras.get(0);
			}
		}
		else if(areTransformsModified) {
			RenderTreeBuilder.rippleTransformations(env);
