...
public class RayTracer {
	 ...
	 */
	public static final int MAX_DEPTH = 12;//12
	...
	public static void shadeRay(Colord outColor, Scene scene, Ray ray, int depth) {
		...
		//System.out.println("record : + " + intersectionRecord.surface);
	...
	public static void renderBlock(Scene scene, Image outImage, int offsetX, int offsetY, int sizeX, int sizeY) {
		...
		for(int x = offsetX; x < (offsetX + sizeX); x++) {
			for(int y = offsetY; y < (offsetY + sizeY); y++) {
				...
				/*for (double dx = - (samples - 1) / 2; dx <= (samples - 1) / 2; dx++) {
					for (double dy = - (samples - 1) / 2; dy <= (samples - 1) / 2; dy++) {*/
				for (double dx = 0; dx < samples; dx++) {
					for (double dy = 0; dy < samples; dy++) {
						//rayColor.setZero();
						double sx = x + 0.5 + dx * sInv;
						double sy = y + 0.5 + dy * sInv;
						
						cam.getRay(ray, sx / width, sy / height);
						//System.out.println("o :" + ray.origin);
						//System.out.println("d :" + ray.direction);
						shadeRay(rayColor, scene, ray, 1); 
						//System.out.println("raycolor + :" + rayColor);
